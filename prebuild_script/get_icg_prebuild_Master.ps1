...
$Codebase_Source = 'C:\Users\mandyhsi\workspace\vieddrv-github\w\camerasw\Master_Sor'
...
if ($cur_branch -eq 'ice_lnl') {
	$branch = 'stable_lnl_pv_20240416'
	write-host 'branch =' $branch
} elseif ($cur_branch -eq 'ice_adl_pv') {
	$branch = 'stable_adlp_pv_cobalt_20210914'
	write-host 'branch =' $branch
} elseif ($cur_branch -eq 'ice_master') {
	$branch = 'master'
	write-host 'branch =' $branch
...
$include = @('Intel3A*','lib*.zip','LNL_*.zip','StaticGraph_LNL.zip','lnl_isp_package.zip','PTL_*.zip','StaticGraph_PTL.zip','ptl_isp_package.zip')
